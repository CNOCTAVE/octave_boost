...
DEFUN_DLD (boost_time_period_contains_time_period, args, nargout,
"-*- texinfo -*-\n\
@deftypefn {Loadable Function} {@var{tf} =} boost_time_period_contains_time_period (@var{period1}, @var{period2})\n\
\n\
Check if @var{period2} is completely contained within @var{period1}. Returns\n\
logical true if @var{period2} is within @var{period1}.\n\
\n\
...
@end itemize\n\
\n\
Based on @code{boost::posix_time::time_period::contains(time_period)}.\n\
\n\
Example:\n\
@example\n\
@group\n\
...
tf = boost_time_period_contains_time_period (tp1, tp2);\n\
@result{} tf = 1 (true)\n\
@end group\n\
@end example\n\
@seealso{boost_time_period_contains_ptime, boost_time_period_intersects, boost_time_period_intersection}\n\
@end deftypefn")
{
  if (args.length () < 2)
    print_usage ();
  ...
  return octave_value (tp1.contains (tp2));
}
