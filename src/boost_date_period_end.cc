...
DEFUN_DLD (boost_date_period_end, args, nargout,
           "-*- texinfo -*-\n\
@deftypefn {Loadable Function} {@var{date} =} boost_date_period_end (@var{period})\n\
\n\
Return one past the last day in the date_period (i.e., the end boundary).\n\
\n\
...
\n\
Based on boost::gregorian::date_period::end().\n\
\n\
Example:\n\
@example\n\
d = boost_date_period_end (2002, 1, 1, 2002, 1, 10)\n\
@result{} struct with year=2002, month=1, day=10\n\
@end example\n\
@end deftypefn")
{
  if (args.length () < 1)
  ...
  return octave_value (date_to_struct (dp.end ()));
}
