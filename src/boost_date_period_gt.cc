...
DEFUN_DLD (boost_date_period_gt, args, nargout,
           "-*- texinfo -*-\n\
@deftypefn {Loadable Function} {@var{tf} =} boost_date_period_gt (@var{period1}, @var{period2})\n\
\n\
Check if @var{period1} is greater than @var{period2}.\n\
\n\
...
@end itemize\n\
\n\
Returns a logical value (1 if period1 > period2, 0 otherwise).\n\
\n\
Based on boost::gregorian::date_period::operator>().\n\
\n\
Example:\n\
@example\n\
tf = boost_date_period_gt (2002, 1, 10, 2002, 1, 30, 2002, 1, 1, 2002, 1, 10)\n\
@result{} 1 (true)\n\
@end example\n\
@end deftypefn")
{
  ...
  if (consumed1 >= args.length ())
    error ("boost_date_period_gt: insufficient arguments for second period");

  auto res2 = parse_date_period (args, consumed1);
  bg::date_period dp2 = res2.period;

  return octave_value (dp1 > dp2);
}
