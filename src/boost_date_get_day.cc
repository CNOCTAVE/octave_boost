...
DEFUN_DLD (boost_date_get_day, args, ,
"-*- texinfo -*-\n\
@deftypefn {Loadable Function} {@var{day} =} boost_date_get_day (@var{date})\n\
\n\
Return the day of the month (1-31) from a Boost.Gregorian date.\n\
\n\
...
@end itemize\n\
\n\
Based on @code{boost::gregorian::date::day()}.\n\
\n\
@example\n\
@group\n\
...
@end deftypefn")
{
  ...
  if (d.is_not_a_date ())
    error ("boost_date_get_day: date is not-a-date-time");

  return octave_value (static_cast<double> (d.day ()));
}
