...
DEFUN_DLD (boost_date_get_day_of_week, args, ,
"-*- texinfo -*-\n\
@deftypefn {Loadable Function} {@var{dow} =} boost_date_get_day_of_week (@var{date})\n\
\n\
...
@end itemize\n\
\n\
Based on @code{boost::gregorian::date::day_of_week()}.\n\
\n\
@example\n\
@group\n\
dow = boost_date_get_day_of_week (2002, 1, 10);\n\
@result{} dow = 4  % Thursday\n\
@end group\n\
@end example\n\
@end deftypefn")
{
  ...
  if (d.is_not_a_date ())
    error ("boost_date_get_day_of_week: date is not-a-date-time");

  return octave_value (static_cast<double> (d.day_of_week ()));
}
