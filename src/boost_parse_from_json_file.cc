...
ptree_to_octave_value (const boost::property_tree::ptree& pt)
{
  ...
  if (is_array)
    {
      // Build a cell array for JSON arrays (children have empty keys)
      size_t n = 0;
      for (const auto& child : pt)
        ++n;
...
DEFUN_DLD (boost_parse_from_json_file, args, nargout,
           "-*- texinfo -*-\n\
@deftypefn {Loadable Function} {@var{data} =} boost_parse_from_json_file (@var{filename})\n\
\n\
Read a JSON format file and parse its content into an Octave data structure.\n\
The file is read using Boost.PropertyTree and converted to an Octave struct.\n\
\n\
Example:\n\
@example\n\
>> data = boost_parse_from_json_file (\"data.json\")\n\
@end example\n\
\n\
@seealso{boost_save_as_json_file, boost_parse_from_info_file, boost_parse_from_ini_file, boost_parse_from_xml_file}\n\
@end deftypefn")
{
  if (args.length () != 1)
    print_usage ();
  ...
  try
    ...
    {
      error ("boost_parse_from_json_file: %s", e.what ());
      return octave_value ();
    }
}
