...
DEFUN_DLD (boost_ptime_to_iso_string, args, ,
"-*- texinfo -*-\n\
@deftypefn {Loadable Function} {@var{str} =} boost_ptime_to_iso_string (@var{ptime})\n\
\n\
Convert a ptime to ISO 8601 format string \"YYYYMMDDTHHMMSS.fffffffff\"\n\
...
@end itemize\n\
\n\
Based on @code{boost::posix_time::to_iso_string()}.\n\
\n\
@example\n\
@group\n\
str = boost_ptime_to_iso_string (\"2002-01-31 10:00:01\");\n\
@result{} str = \"20020131T100001\"\n\
@end group\n\
@end example\n\
@end deftypefn")
{
  ...
  if (t.is_not_a_date_time ())
    error ("boost_ptime_to_iso_string: ptime is not-a-date-time");

  return octave_value (bp::to_iso_string (t));
}
