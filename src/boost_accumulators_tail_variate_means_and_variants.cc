...
DEFUN_DLD (boost_accumulators_tail_variate_means_and_variants, args, ,
"-*- texinfo -*-\n\
@deftypefn {Loadable Function} {@var{result} =} boost_accumulators_tail_variate_means_and_variants (@var{data}, @var{cache_size})\n\
\n\
Estimate the absolute and relative tail variate means for both left\n\
and right tails using Boost.Accumulators.\n\
@var{data} must be an Nx5 matrix where each row contains 5 variates.\n\
@var{cache_size} (default 5) specifies the tail cache size.\n\
...
r = boost_accumulators_tail_variate_means_and_variants (data, 5);\n\
@end group\n\
@end example\n\
@end deftypefn")
{
  ...
  NDArray data = args(0).xarray_value ("boost_accumulators_tail_variate_means_and_variants: "
                                       "DATA must be a numeric matrix");

  if (data.columns () < 2)
    error ("boost_accumulators_tail_variate_means_and_variants: "
           "DATA must have at least 2 columns (sample + variates)");

  std::size_t cache_size = 5;
  if (args.length () >= 2)
    {
      cache_size = args(1).xuint64_value ("boost_accumulators_tail_variate_means_and_variants: "
  ...
  // The number of variates is columns - 1 (first column is the sample value)
  octave_idx_type n_variates = data.columns () - 1;

  // We use std::vector for the variate set type
  ...
  for (octave_idx_type i = 0; i < n; ++i)
    {
      ...
      acc_rel_right (sample, covariate1 = cov);
      acc_abs_right (sample, covariate1 = cov);
      acc_rel_left (sample, covariate1 = cov);
      acc_abs_left (sample, covariate1 = cov);
    }
  ...
  auto rel_right = relative_tail_variate_means (acc_rel_right, quantile_probability = 0.7);
  result.setfield ("relative_tail_variate_means_right",
                   octave_value (to_ndarray (rel_right)));

  // Right tail: absolute
  auto abs_right = tail_variate_means (acc_abs_right, quantile_probability = 0.7);
  ...
  auto rel_left = relative_tail_variate_means (acc_rel_left, quantile_probability = 0.3);
  result.setfield ("relative_tail_variate_means_left",
                   octave_value (to_ndarray (rel_left)));

  // Left tail: absolute
  auto abs_left = tail_variate_means (acc_abs_left, quantile_probability = 0.3);
