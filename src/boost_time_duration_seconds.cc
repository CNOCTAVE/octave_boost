...
DEFUN_DLD (boost_time_duration_seconds, args, nargout,
"-*- texinfo -*-\n\
@deftypefn {Loadable Function} {@var{seconds} =} boost_time_duration_seconds (@var{td})\n\
\n\
Get the normalized number of seconds +/- (0..59) from a time duration.\n\
\n\
...
\n\
Based on @code{boost::posix_time::time_duration::seconds()}.\n\
\n\
@example\n\
@group\n\
boost_time_duration_seconds (1, 2, 3)\n\
@result{} 3\n\
@end group\n\
@end example\n\
@end deftypefn")
{
  ...
  if (td.is_special ())
    error ("boost_time_duration_seconds: time_duration is a special value");

  return octave_value (static_cast<double> (td.seconds ()));
}
