...
DEFUN_DLD (boost_date_period_last, args, nargout,
           "-*- texinfo -*-\n\
@deftypefn {Loadable Function} {@var{date} =} boost_date_period_last (@var{period})\n\
\n\
Return the last day of a date_period.\n\
\n\
...
\n\
Based on boost::gregorian::date_period::last().\n\
\n\
Example:\n\
@example\n\
d = boost_date_period_last (2002, 1, 1, 2002, 1, 10)\n\
@result{} struct with year=2002, month=1, day=9\n\
@end example\n\
@end deftypefn")
{
  if (args.length () < 1)
  ...
  return octave_value (date_to_struct (dp.last ()));
}
