                       ...
                       const std::string& key = "")
{
  if (ov.isstruct ())
    ...
    {
      // Must check string BEFORE is_matrix_type(), because Octave strings
      // (char arrays) also satisfy is_matrix_type() — that would cause
      // "invalid conversion from string to real N-D array" if handled as numeric.
...
DEFUN_DLD (boost_save_as_json_file, args, nargout,
           "-*- texinfo -*-\n\
@deftypefn {Loadable Function} {@var{tf} =} boost_save_as_json_file (@var{data}, @var{filename})\n\
\n\
Save Octave data (struct, numeric, string, or cell array) as a JSON format file.\n\
The data is converted to a Boost.PropertyTree and written to the file.\n\
\n\
Example:\n\
@example\n\
>> data = struct ('name', \"Alice\", 'age', 30, 'score', 95.5);\n\
>> boost_save_as_json_file (data, \"data.json\")\n\
@end example\n\
\n\
@seealso{boost_parse_from_json_file, boost_save_as_info_file, boost_save_as_ini_file, boost_save_as_xml_file}\n\
@end deftypefn")
{
  if (args.length () != 2)
    print_usage ();
  ...
  try
    ...
    {
      error ("boost_save_as_json_file: %s", e.what ());
