...
DEFUN_DLD (boost_date_is_not_a_date, args, ,
"-*- texinfo -*-\n\
@deftypefn {Loadable Function} {@var{tf} =} boost_date_is_not_a_date (@var{date})\n\
\n\
Return true if the given Boost.Gregorian date is not-a-date-time.\n\
\n\
...
@end itemize\n\
\n\
Based on @code{boost::gregorian::date::is_not_a_date()}.\n\
\n\
@example\n\
@group\n\
tf = boost_date_is_not_a_date ('not_a_date_time');\n\
@result{} tf = 1 (true)\n\
\n\
tf = boost_date_is_not_a_date (2002, 1, 10);\n\
@result{} tf = 0 (false)\n\
@end group\n\
@end example\n\
@end deftypefn")
{
  ...
  return octave_value (d.is_not_a_date ());
}
