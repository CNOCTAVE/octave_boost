...
DEFUN_DLD (boost_time_duration_minus_time_duration, args, nargout,
"-*- texinfo -*-\n\
@deftypefn {Loadable Function} {@var{td} =} boost_time_duration_minus_time_duration (@var{td1}, @var{td2})\n\
\n\
Subtract td2 from td1 and return the result as a new time_duration.\n\
\n\
...
@example\n\
@group\n\
td = boost_time_duration_minus_time_duration (2, 30, 0, 1, 45, 0);\n\
@result{} td.hours = 0, td.minutes = 45, td.seconds = 0\n\
@end group\n\
@end example\n\
@seealso{boost_time_duration_plus_time_duration, boost_chrono_duration_minus, boost_time_duration_hours}\n\
@end deftypefn")
{
  if (args.length () < 2)
    print_usage ();
