...
DEFUN_DLD (boost_time_period_length, args, nargout,
"-*- texinfo -*-\n\
@deftypefn {Loadable Function} {@var{td} =} boost_time_period_length (@var{period})\n\
\n\
Return the length of a time_period as a time_duration.\n\
\n\
...
\n\
Based on @code{boost::posix_time::time_period::length()}.\n\
\n\
Example:\n\
@example\n\
@group\n\
td = boost_time_period_length (struct ('begin', '2002-01-10 00:00:00', 'length', struct ('hours', 1, 'minutes', 30, 'seconds', 0)));\n\
@result{} td.hours = 1, td.minutes = 30, td.seconds = 0\n\
@end group\n\
@end example\n\
@seealso{boost_time_period_begin, boost_time_period_end}\n\
@end deftypefn")
{
  if (args.length () < 1)
    print_usage ();
  ...
  return octave_value (time_duration_to_struct (tp.length ()));
}
