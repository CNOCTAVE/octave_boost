...
DEFUN_DLD (boost_time_duration_ne, args, ,
"-*- texinfo -*-\n\
@deftypefn {Loadable Function} {@var{tf} =} boost_time_duration_ne (@var{td1}, @var{td2})\n\
\n\
Compare two time durations for inequality.\n\
\n\
...
@example\n\
@group\n\
boost_time_duration_ne (1, 2, 3, 4, 5, 6)\n\
@result{} 1\n\
\n\
boost_time_duration_ne (1, 2, 3, 1, 2, 3)\n\
@result{} 0\n\
@end group\n\
@end example\n\
@end deftypefn")
{
  ...
  if (args (0).isstruct () || args (0).is_string ())
    ...
    error ("boost_time_duration_ne: invalid arguments for first time_duration");
