...
DEFUN_DLD (boost_date_period_span, args, nargout,
           "-*- texinfo -*-\n\
@deftypefn {Loadable Function} {@var{period_out} =} boost_date_period_span (@var{period1}, @var{period2})\n\
\n\
Combines two periods and any gap between them such that\n\
begin = min (p1.begin, p2.begin) and end = max (p1.end, p2.end).\n\
\n\
...
\n\
Based on boost::gregorian::date_period::span().\n\
\n\
Example:\n\
@example\n\
dp = boost_date_period_span (2002, 1, 1, 2002, 1, 5, 2002, 1, 9, 2002, 1, 31)\n\
...
@end deftypefn")
{
  ...
  if (consumed1 >= args.length ())
    error ("boost_date_period_span: insufficient arguments for second period");

  auto res2 = parse_date_period (args, consumed1);
  bg::date_period dp2 = res2.period;

  bg::date_period result = dp1.span (dp2);
