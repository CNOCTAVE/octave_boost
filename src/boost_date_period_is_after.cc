...
DEFUN_DLD (boost_date_period_is_after, args, nargout,
           "-*- texinfo -*-\n\
@deftypefn {Loadable Function} {@var{tf} =} boost_date_period_is_after (@var{period}, @var{date})\n\
\n\
Determine if the date_period is after a given date.\n\
\n\
...
\n\
Returns a logical value (1 if period is after the date, 0 otherwise).\n\
\n\
Based on boost::gregorian::date_period::is_after().\n\
\n\
Example:\n\
@example\n\
tf = boost_date_period_is_after (2002, 1, 10, 2002, 1, 30, 2002, 1, 3)\n\
@result{} 1 (true)\n\
@end example\n\
@end deftypefn")
{
  ...
  if (consumed >= args.length ())
    error ("boost_date_period_is_after: missing date argument");

  bg::date d = parse_date (args, consumed);

  return octave_value (dp.is_after (d));
}
