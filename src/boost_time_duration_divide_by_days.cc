...
DEFUN_DLD (boost_time_duration_divide_by_days, args, nargout,
"-*- texinfo -*-\n\
@deftypefn {Loadable Function} {@var{td} =} boost_time_duration_divide_by_days (@var{td}, @var{divisor})\n\
\n\
Divide a time_duration by an integer divisor and return the result.\n\
\n\
...
@end itemize\n\
\n\
The @var{divisor} is an integer scalar.\n\
\n\
...
@example\n\
@group\n\
td = boost_time_duration_divide_by_days (3, 0, 0, 2);\n\
@result{} td.hours = 1, td.minutes = 30, td.seconds = 0\n\
@end group\n\
@end example\n\
@seealso{boost_time_duration_multiply_by_days, boost_time_duration_hours}\n\
@end deftypefn")
{
  if (args.length () < 2)
    print_usage ();
  ...
  double divisor = args(n1).double_value ();
  if (divisor == 0.0)
    error ("boost_time_duration_divide_by_days: division by zero");

  bp::time_duration result = td / static_cast<long> (divisor);
