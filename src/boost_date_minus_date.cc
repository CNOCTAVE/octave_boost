...
DEFUN_DLD (boost_date_minus_date, args, nargout,
           "-*- texinfo -*-\n\
@deftypefn {Loadable Function} {@var{days} =} boost_date_minus_date (@var{date1}, @var{date2})\n\
\n\
Compute the difference between two dates in days (date1 - date2).\n\
\n\
...
@end itemize\n\
\n\
Returns the difference as a number of days (positive if date1 > date2).\n\
\n\
Based on boost::gregorian::date - date.\n\
\n\
Example:\n\
@example\n\
boost_date_minus_date (2002, 1, 11, 2002, 1, 10)\n\
@result{} 1\n\
boost_date_minus_date (2002, 1, 10, 2002, 1, 11)\n\
@result{} -1\n\
@end example\n\
@end deftypefn")
{
  ...
  return octave_value (static_cast<double> (dd.days ()));
}
