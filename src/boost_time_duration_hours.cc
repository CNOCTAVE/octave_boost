...
DEFUN_DLD (boost_time_duration_hours, args, nargout,
"-*- texinfo -*-\n\
@deftypefn {Loadable Function} {@var{hours} =} boost_time_duration_hours (@var{td})\n\
\n\
Get the number of normalized hours from a time duration.\n\
\n\
...
\n\
Based on @code{boost::posix_time::time_duration::hours()}.\n\
\n\
@example\n\
@group\n\
boost_time_duration_hours (1, 2, 3)\n\
@result{} 1\n\
@end group\n\
@end example\n\
@end deftypefn")
{
  ...
  if (td.is_special ())
    error ("boost_time_duration_hours: time_duration is a special value");

  return octave_value (static_cast<double> (td.hours ()));
}
