...
DEFUN_DLD (boost_time_duration_is_positive, args, nargout,
"-*- texinfo -*-\n\
@deftypefn {Loadable Function} {@var{tf} =} boost_time_duration_is_positive (@var{td})\n\
\n\
Return true if the time duration is positive, false otherwise.\n\
\n\
...
@end itemize\n\
\n\
Based on @code{boost::posix_time::time_duration::is_positive()}.\n\
\n\
@example\n\
@group\n\
boost_time_duration_is_positive (1, 0, 0)\n\
@result{} 1\n\
@end group\n\
@end example\n\
@end deftypefn")
{
  ...
  return octave_value (td.is_positive ());
}
