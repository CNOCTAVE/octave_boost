...
DEFUN_DLD (boost_date_period_merge, args, nargout,
           "-*- texinfo -*-\n\
@deftypefn {Loadable Function} {@var{period_out} =} boost_date_period_merge (@var{period1}, @var{period2})\n\
\n\
Returns the union of two date_periods. Null if no intersection.\n\
\n\
...
If the periods do not intersect, returns a struct with @qcode{\"length\"} = 0.\n\
\n\
Based on boost::gregorian::date_period::merge().\n\
\n\
Example:\n\
@example\n\
dp = boost_date_period_merge (2002, 1, 1, 2002, 1, 10, 2002, 1, 9, 2002, 1, 31)\n\
@result{} period: 2002-Jan-01 to 2002-Jan-31\n\
@end example\n\
@end deftypefn")
{
  if (args.length () < 2)
  ...
  if (consumed1 >= args.length ())
    error ("boost_date_period_merge: insufficient arguments for second period");

  auto res2 = parse_date_period (args, consumed1);
  bg::date_period dp2 = res2.period;

  bg::date_period result = dp1.merge (dp2);
