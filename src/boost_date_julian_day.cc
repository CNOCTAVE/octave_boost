...
DEFUN_DLD (boost_date_julian_day, args, ,
"-*- texinfo -*-\n\
@deftypefn {Loadable Function} {@var{jd} =} boost_date_julian_day (@var{date})\n\
\n\
Return the Julian day number for the given Boost.Gregorian date.\n\
\n\
...
@end itemize\n\
\n\
Based on @code{boost::gregorian::date::julian_day()}.\n\
\n\
@example\n\
@group\n\
...
@end deftypefn")
{
  ...
  if (d.is_not_a_date ())
    error ("boost_date_julian_day: date is not-a-date-time");

  return octave_value (static_cast<double> (d.julian_day ()));
}
