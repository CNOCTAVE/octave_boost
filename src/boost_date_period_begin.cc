...
DEFUN_DLD (boost_date_period_begin, args, nargout,
           "-*- texinfo -*-\n\
@deftypefn {Loadable Function} {@var{date} =} boost_date_period_begin (@var{period})\n\
\n\
Return the first day (begin) of a date_period.\n\
\n\
...
\n\
Based on boost::gregorian::date_period::begin().\n\
\n\
Example:\n\
@example\n\
d = boost_date_period_begin (2002, 1, 1, 2002, 1, 10)\n\
@result{} struct with year=2002, month=1, day=1\n\
@end example\n\
@end deftypefn")
{
  if (args.length () < 1)
  ...
  return octave_value (date_to_struct (dp.begin ()));
}
