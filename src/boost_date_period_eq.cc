...
DEFUN_DLD (boost_date_period_eq, args, nargout,
           "-*- texinfo -*-\n\
@deftypefn {Loadable Function} {@var{tf} =} boost_date_period_eq (@var{period1}, @var{period2})\n\
\n\
Check if two date_periods are equal.\n\
\n\
...
@end itemize\n\
\n\
Returns a logical value (1 if equal, 0 otherwise).\n\
\n\
Based on boost::gregorian::date_period::operator==().\n\
\n\
Example:\n\
@example\n\
tf = boost_date_period_eq (2002, 1, 1, 2002, 1, 10, 2002, 1, 1, 2002, 1, 10)\n\
@result{} 1 (true)\n\
@end example\n\
@end deftypefn")
{
  ...
  if (consumed1 >= args.length ())
    error ("boost_date_period_eq: insufficient arguments for second period");
