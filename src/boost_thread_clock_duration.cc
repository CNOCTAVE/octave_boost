...
DEFUN_DLD (boost_thread_clock_duration, args, ,
"-*- texinfo -*-\n\
@deftypefn {Loadable Function} {@var{dur} =} boost_thread_clock_duration ()\n\
\n\
Measure the elapsed thread CPU time between entering and exiting this\n\
function using boost::chrono::thread_clock.  The thread clock measures CPU\n\
time consumed by the current thread.  The function enters an infinite loop;\n\
...
@example\n\
@group\n\
dur = boost_thread_clock_duration ();\n\
...
@end deftypefn")
{
  ...
  auto end = boost::chrono::thread_clock::now ();
