...
DEFUN_DLD (boost_time_duration_minutes, args, nargout,
"-*- texinfo -*-\n\
@deftypefn {Loadable Function} {@var{minutes} =} boost_time_duration_minutes (@var{td})\n\
\n\
Get the number of normalized minutes +/- (0..59) from a time duration.\n\
\n\
...
\n\
Based on @code{boost::posix_time::time_duration::minutes()}.\n\
\n\
@example\n\
@group\n\
boost_time_duration_minutes (1, 2, 3)\n\
@result{} 2\n\
@end group\n\
@end example\n\
@end deftypefn")
{
  ...
  if (td.is_special ())
    error ("boost_time_duration_minutes: time_duration is a special value");

  return octave_value (static_cast<double> (td.minutes ()));
}
