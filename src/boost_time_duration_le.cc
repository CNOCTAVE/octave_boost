...
DEFUN_DLD (boost_time_duration_le, args, ,
"-*- texinfo -*-\n\
@deftypefn {Loadable Function} {@var{tf} =} boost_time_duration_le (@var{td1}, @var{td2})\n\
\n\
...
@example\n\
@group\n\
boost_time_duration_le (1, 0, 0, 5, 0, 0)\n\
@result{} 1\n\
\n\
boost_time_duration_le (1, 0, 0, 1, 0, 0)\n\
@result{} 1\n\
\n\
boost_time_duration_le (5, 0, 0, 1, 0, 0)\n\
@result{} 0\n\
@end group\n\
@end example\n\
@end deftypefn")
{
  ...
  if (args (0).isstruct () || args (0).is_string ())
    ...
    error ("boost_time_duration_le: invalid arguments for first time_duration");
