                       ...
                       const std::string& key = "")
{
  ...
  // IMPORTANT: is_string() check must precede is_matrix_type()/isnumeric() check
  // because isnumeric() may return true for string objects in some Octave versions,
  // causing "invalid conversion from string to real N-D array" errors when saving
  // struct data parsed from JSON.
...
DEFUN_DLD (boost_save_as_ini_file, args, nargout,
           "-*- texinfo -*-\n\
@deftypefn {Loadable Function} {@var{tf} =} boost_save_as_ini_file (@var{data}, @var{filename})\n\
\n\
Save Octave data (struct, numeric, string, or cell array) as an INI format file.\n\
The data is converted to a Boost.PropertyTree and written to the file.\n\
\n\
Example:\n\
@example\n\
>> data = struct ('name', \"Alice\", 'age', 30);\n\
>> boost_save_as_ini_file (data, \"data.ini\")\n\
@end example\n\
\n\
@seealso{boost_parse_from_ini_file, boost_save_as_info_file, boost_save_as_json_file, boost_save_as_xml_file}\n\
@end deftypefn")
{
  if (args.length () != 2)
    print_usage ();
  ...
  try
    ...
    {
      error ("boost_save_as_ini_file: %s", e.what ());
