...
DEFUN_DLD (boost_chrono_steady_clock_now, args, ,
"-*- texinfo -*-\n\
@deftypefn {Loadable Function} {@var{time} =} boost_chrono_steady_clock_now ()\n\
\n\
Return the current steady clock time as a struct with fields @qcode{\"count\"}\n\
and @qcode{\"unit\"}. The steady clock is monotonic and is not affected by\n\
wall-clock adjustments. The count is in nanoseconds since the clock's epoch.\n\
Based on boost::chrono::steady_clock::now().\n\
\n\
@example\n\
@group\n\
t = boost_chrono_steady_clock_now ();\n\
