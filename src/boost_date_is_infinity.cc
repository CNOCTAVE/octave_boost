...
DEFUN_DLD (boost_date_is_infinity, args, ,
"-*- texinfo -*-\n\
@deftypefn {Loadable Function} {@var{tf} =} boost_date_is_infinity (@var{date})\n\
\n\
Return true if the given Boost.Gregorian date is infinity (positive or negative).\n\
\n\
...
@end itemize\n\
\n\
Based on @code{boost::gregorian::date::is_infinity()}.\n\
\n\
@example\n\
@group\n\
tf = boost_date_is_infinity ('pos_infin');\n\
@result{} tf = 1 (true)\n\
\n\
tf = boost_date_is_infinity (2002, 1, 10);\n\
@result{} tf = 0 (false)\n\
@end group\n\
@end example\n\
@end deftypefn")
{
  ...
  return octave_value (d.is_infinity ());
}
