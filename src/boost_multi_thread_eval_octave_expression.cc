...
static boost::mutex octave_mutex2;
...
void eval_expression_thread (const std::string& expression,
                             EvalThreadResult& result)
{
  try
    {
      // Get thread ID
      ...
      {
        boost::mutex::scoped_lock lock (octave_mutex2);

        // Use evalc to capture output from eval()
        // Build expression: "[output, result] = evalc('expression')"
        ...
        // Store the actual octave_value directly instead of serializing to string
...
DEFUN_DLD (boost_multi_thread_eval_octave_expression, args, nargout,
           "-*- texinfo -*-\n\
@deftypefn  {Loadable Function} {@var{result} =} boost_multi_thread_eval_octave_expression (@var{expression})\n\
@deftypefnx {Loadable Function} {@var{result} =} boost_multi_thread_eval_octave_expression (@var{expression}, @var{num_threads})\n\
\n\
启动多个 boost::thread，并行 eval() 指定的 Octave 表达式。\n\
...
示例：\n\
@example\n\
result = boost_multi_thread_eval_octave_expression (\"rand()\", 4);\n\
@end example\n\
\n\
@seealso{boost_multi_thread_run_octave_file, boost_multi_thread_call_octave_function, boost_multi_thread_feval_octave_expression}\n\
@end deftypefn")
{
  try
    {
      ...
      // First argument: expression string
      if (! args(0).is_string ())
        error ("boost_multi_thread_eval_octave_expression: EXPRESSION must be a string");
      std::string expression = args(0).string_value ();
      ...
      if (nargin >= 2)
        {
          if (! args(1).is_scalar_type () || ! args(1).isnumeric ())
            error ("boost_multi_thread_eval_octave_expression: NUM_THREADS must be a positive integer");
      ...
      std::vector<EvalThreadResult> results (num_threads);
      boost::thread_group tg;

      // Launch threads
      for (int i = 0; i < num_threads; ++i)
        {
          tg.create_thread (boost::bind (eval_expression_thread,
                                         expression,
                                         boost::ref (results[i])));
        }
      ...
      for (int i = 0; i < num_threads; ++i)
        {
          ...
          result_cell(i) = results[i].func_result;
    ...
    {
      error ("boost_multi_thread_eval_octave_expression: %s", e.what ());
      return octave_value ();
    }
}
