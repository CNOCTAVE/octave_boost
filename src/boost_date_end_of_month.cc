...
DEFUN_DLD (boost_date_end_of_month, args, ,
"-*- texinfo -*-\n\
@deftypefn {Loadable Function} {@var{date} =} boost_date_end_of_month (@var{date})\n\
\n\
...
@example\n\
@group\n\
eom = boost_date_end_of_month (2000, 2, 1);\n\
@result{} eom.year = 2000, eom.month = 2, eom.day = 29\n\
@end group\n\
@end example\n\
@end deftypefn")
{
  ...
  if (d.is_not_a_date ())
    error ("boost_date_end_of_month: date is not-a-date-time");
