...
DEFUN_DLD (boost_time_duration_ticks, args, nargout,
"-*- texinfo -*-\n\
@deftypefn {Loadable Function} {@var{ticks} =} boost_time_duration_ticks (@var{td})\n\
\n\
Return the raw count of the time duration type (the number of ticks).\n\
\n\
...
@end itemize\n\
\n\
Based on @code{boost::posix_time::time_duration::ticks()}.\n\
\n\
@example\n\
@group\n\
boost_time_duration_ticks (0, 0, 0, 1000)\n\
@result{} 1000\n\
@end group\n\
@end example\n\
@end deftypefn")
{
  ...
  if (td.is_special ())
    error ("boost_time_duration_ticks: time_duration is a special value");

  return octave_value (static_cast<double> (td.ticks ()));
}
