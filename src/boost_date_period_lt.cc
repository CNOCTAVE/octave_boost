...
DEFUN_DLD (boost_date_period_lt, args, nargout,
           "-*- texinfo -*-\n\
@deftypefn {Loadable Function} {@var{tf} =} boost_date_period_lt (@var{period1}, @var{period2})\n\
\n\
Check if @var{period1} is less than @var{period2}.\n\
\n\
...
Example:\n\
@example\n\
tf = boost_date_period_lt (2002, 1, 1, 2002, 1, 10, 2002, 1, 10, 2002, 1, 30)\n\
@result{} 1 (true)\n\
@end example\n\
@end deftypefn")
{
  ...
  if (consumed1 >= args.length ())
    error ("boost_date_period_lt: insufficient arguments for second period");
