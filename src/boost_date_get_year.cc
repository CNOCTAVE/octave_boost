...
DEFUN_DLD (boost_date_get_year, args, ,
"-*- texinfo -*-\n\
@deftypefn {Loadable Function} {@var{year} =} boost_date_get_year (@var{date})\n\
\n\
Return the year from a Boost.Gregorian date.\n\
\n\
...
@end itemize\n\
\n\
Based on @code{boost::gregorian::date::year()}.\n\
\n\
@example\n\
@group\n\
...
@end deftypefn")
{
  ...
  if (d.is_not_a_date ())
    error ("boost_date_get_year: date is not-a-date-time");

  return octave_value (static_cast<double> (d.year ()));
}
