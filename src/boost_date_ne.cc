...
DEFUN_DLD (boost_date_ne, args, nargout,
           "-*- texinfo -*-\n\
@deftypefn {Loadable Function} {@var{tf} =} boost_date_ne (@var{date1}, @var{date2})\n\
\n\
Compare two dates for inequality (not equal).\n\
\n\
...
Example:\n\
@example\n\
boost_date_ne (2002, 1, 10, 2002, 1, 11)\n\
@result{} 1 (true)\n\
boost_date_ne (2002, 1, 10, 2002, 1, 10)\n\
@result{} 0 (false)\n\
@end example\n\
@end deftypefn")
{
  ...
  // Count arguments for first date
  if (args(0).isstruct () || args(0).is_string ())
    n1 = 1;
  else
    n1 = 3;

  // Count arguments for second date - check remaining args
  int remaining = args.length () - n1;
  if (args(n1).isstruct () || args(n1).is_string ())
    n2 = 1;
  else if (remaining >= 3)
    n2 = 3;
  else
