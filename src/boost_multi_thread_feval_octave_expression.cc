...
static boost::mutex octave_mutex3;

// Structure to hold per-thread results
struct FevalThreadResult
{
...
void feval_expression_thread (const std::string& funcname,
                              const octave_value_list& args_list,
                              FevalThreadResult& result)
{
  try
    {
      // Get thread ID
      ...
      // Lock Octave interpreter for thread safety
      {
        ...
        std::string feval_expr = funcname + "(__octave_boost_feval_args__{:})";
...
DEFUN_DLD (boost_multi_thread_feval_octave_expression, args, nargout,
           "-*- texinfo -*-\n\
@deftypefn  {Loadable Function} {@var{result} =} boost_multi_thread_feval_octave_expression (@var{funcname})\n\
@deftypefnx {Loadable Function} {@var{result} =} boost_multi_thread_feval_octave_expression (@var{funcname}, @var{num_threads})\n\
@deftypefnx {Loadable Function} {@var{result} =} boost_multi_thread_feval_octave_expression (@var{funcname}, @var{num_threads}, @var{args})\n\
\n\
启动多个 boost::thread，并行 feval() 指定的 Octave 函数。\n\
...
示例：\n\
@example\n\
result = boost_multi_thread_feval_octave_expression (\"sin\", 4, {pi/2});\n\
@end example\n\
\n\
@seealso{boost_multi_thread_run_octave_file, boost_multi_thread_call_octave_function, boost_multi_thread_eval_octave_expression}\n\
@end deftypefn")
{
  try
    {
      ...
      // First argument: function name for feval
      if (! args(0).is_string ())
        error ("boost_multi_thread_feval_octave_expression: FUNCNAME must be a string");
      std::string funcname = args(0).string_value ();
      ...
      if (nargin >= 2)
        {
          if (! args(1).is_scalar_type () || ! args(1).isnumeric ())
            error ("boost_multi_thread_feval_octave_expression: NUM_THREADS must be a positive integer");
          num_threads = args(1).int_value ();
          if (num_threads < 1)
            error ("boost_multi_thread_feval_octave_expression: NUM_THREADS must be a positive integer");
        }

      // Third argument (optional): feval arguments (cell array, default empty)
      octave_value_list func_args;
      if (nargin >= 3)
        {
          if (! args(2).iscell ())
            {
              error ("boost_multi_thread_feval_octave_expression: ARGS must be a cell array");
              return octave_value ();
            }
      ...
      std::vector<FevalThreadResult> results (num_threads);
      boost::thread_group tg;

      // Launch threads
      for (int i = 0; i < num_threads; ++i)
        {
          tg.create_thread (boost::bind (feval_expression_thread,
                                         funcname,
                                         func_args,
                                         boost::ref (results[i])));
        }
    ...
    {
      error ("boost_multi_thread_feval_octave_expression: %s", e.what ());
      return octave_value ();
    }
}
