...
DEFUN_DLD (boost_date_period_shift, args, nargout,
           "-*- texinfo -*-\n\
@deftypefn {Loadable Function} {@var{period_out} =} boost_date_period_shift (@var{period}, @var{days})\n\
\n\
Add a duration to both begin and end of a date_period, returning a new period.\n\
\n\
...
@end itemize\n\
The @var{days} parameter is an integer number of days to shift.\n\
\n\
Returns a struct with fields @qcode{\"begin\"}, @qcode{\"last\"}, @qcode{\"end\"}, @qcode{\"length\"}.\n\
\n\
Based on boost::gregorian::date_period::shift().\n\
\n\
Example:\n\
@example\n\
dp = boost_date_period_shift (2005, 1, 1, 3, 3)\n\
@result{} period: 2005-Jan-04 to 2005-Jan-07\n\
@end example\n\
@end deftypefn")
{
  if (args.length () < 2)
  ...
  if (consumed >= args.length ())
    error ("boost_date_period_shift: missing days argument");

  long ndays = args (consumed).long_value ();

  dp.shift (bg::days (ndays));
