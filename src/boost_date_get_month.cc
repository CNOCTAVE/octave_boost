...
DEFUN_DLD (boost_date_get_month, args, ,
"-*- texinfo -*-\n\
@deftypefn {Loadable Function} {@var{month} =} boost_date_get_month (@var{date})\n\
\n\
Return the month (1-12) from a Boost.Gregorian date.\n\
\n\
...
@end itemize\n\
\n\
Based on @code{boost::gregorian::date::month()}.\n\
\n\
@example\n\
@group\n\
...
@end deftypefn")
{
  ...
  if (d.is_not_a_date ())
    error ("boost_date_get_month: date is not-a-date-time");

  return octave_value (static_cast<double> (d.month ()));
}
