...
DEFUN_DLD (boost_date_week_number, args, ,
"-*- texinfo -*-\n\
@deftypefn {Loadable Function} {@var{wn} =} boost_date_week_number (@var{date})\n\
\n\
Return the ISO 8601 week number for the given Boost.Gregorian date.\n\
\n\
...
@end itemize\n\
\n\
Based on @code{boost::gregorian::date::week_number()}.\n\
\n\
@example\n\
@group\n\
...
@end deftypefn")
{
  ...
  if (d.is_not_a_date ())
    error ("boost_date_week_number: date is not-a-date-time");

  return octave_value (static_cast<double> (d.week_number ()));
}
