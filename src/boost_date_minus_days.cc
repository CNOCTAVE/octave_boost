...
DEFUN_DLD (boost_date_minus_days, args, nargout,
           "-*- texinfo -*-\n\
@deftypefn {Loadable Function} {@var{date_out} =} boost_date_minus_days (@var{date}, @var{days})\n\
\n\
Subtract a number of days from a date and return the new date.\n\
\n\
...
@end itemize\n\
The @var{days} parameter is an integer number of days to subtract.\n\
\n\
...
Example:\n\
@example\n\
boost_date_minus_days (2002, 1, 10, 5)\n\
@result{} struct with year=2002, month=1, day=5\n\
@end example\n\
@end deftypefn")
{
  if (args.length () < 2)
